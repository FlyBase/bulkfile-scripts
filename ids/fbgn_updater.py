    ...
    with open(fbid_file, "r") as file:
        for line in file:
            ...
            if not line.startswith("#") and "FBgn" in line:
                ...
                [
                    insert_fbid(primary_fbid, fbid, fbid_dict)
                    for fbid in secondary_fbid_list
                ]
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Usage: {os.path.basename(__file__)} your_FBgn_ids.txt fbgn_annotation_ID.tsv",
            file=sys.stderr,
        )
